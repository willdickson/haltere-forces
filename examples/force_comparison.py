...
# General force calculations
hsim = Halteres(param=param)
omega_x = np.deg2rad(300.0)
omega = np.array([omega_x,   0.0,   0.0])
hsim_force = hsim.force(omega)
...
# Force calculation from simplified formula
...
ax[2].plot(t, f, 'b')
ax[2].plot(hsim.t, hsim_force['right']['lateral']['coriolis'], 'g')
ax[2].set_ylabel('lat. coriolis (N)')
ax[2].grid(True)
